...
_params = {
    ...
    # Genetic Algorithm
    "GeneticAlgorithm": {"fitness_func": type(lambda: None)},
    # Crossover Params
    "Crossover": {
    ...
    # Misc Params
    "Misc": {"level": str, "allow_additional_time_for_flagpole": bool},
}
            ...
            for k, v in self._config_dict[section].items():
                ...
                if isinstance(_type, tuple):
                    if len(_type) == 2:
                        ...
                        raise Exception(
                            "Expected a 2 tuple value describing that it is to be parse as a tuple and the type to cast it as"
                        )
                elif "lambda" in v:
                    try:
    ...
    def _verify_sections(self) -> None:
        ...
        for section in self._config.sections():
            ...
            if section not in _params:
                raise Exception(
                    'Section "{}" has no parameters allowed. Please remove this section and run again.'.format(section)
                )
