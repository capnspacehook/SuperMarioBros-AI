...
class Visualizer(QtWidgets.QWidget):
    ...
    def _draw_region_of_interest(self, painter: QPainter) -> None:
        ...
        painter.drawRect(
            x * self.tile_width + 5 + self.x_offset,
            start_row * self.tile_height + 5,
            viz_width * self.tile_width,
            viz_height * self.tile_height,
        )
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, config: Optional[Config] = None):
        ...
        if args.no_display:
            ...
            if args.load_inds:
                ...
                )  # +1 because we switch from 0 to 1 index
    ...
    def keyReleaseEvent(self, event):
        k = event.key()
        m = {Qt.Key_Right: 7, Qt.Key_C: 8, Qt.Key_X: 0, Qt.Key_Left: 6, Qt.Key_Down: 5}
        if k in m:
            self.keys[m[k]] = 0
        ...
        if self.mario.is_alive:
            ...
            if args.replay_file:
                ...
                if (self.current_generation > self._true_zero_gen and self._current_individual == self._next_gen_size) or (
                    self.current_generation == self._true_zero_gen
                    and self._current_individual == self.config.Selection.num_parents
                ):
